...
bool Robot::close(double x, double y, double a)
{
    double x1 = getPose().px;
    double y1 = getPose().py;

    return distance(x, x1, y, y1) < 5;
}//returns true when the robot is close to the goal.
...
Robot::Robot(int port, double startX, double startY, double startYaw)
{
    ...
    robot->Read();
    speechProxy->Say("Player Server Established!");
    fileName = "Log.txt";
    return;
}//end constructor.
...
void Robot::planPath()
{
    ...
    plannerProxy->SetGoalPose(10, -28, dtor(180));
    ...
    plannerProxy->SetGoalPose(-13, -28, dtor(90));
    plannerProxy->RequestWaypoints();
    freaked = false;
    //planner->Read();
    robot->Read();

    while (!done || !close(-13, -28, 90))
    {
        robot->Read();
        //planner->Read();
        done = plannerProxy->GetPathDone();
    ...
    freaked = false;
    plannerProxy->SetGoalPose(-6.778, 19.518, dtor(-90));
    plannerProxy->RequestWaypoints();
    while (!done || !close(-6.778, 19.518, dtor(-90)))
    {
        robot->Read();
        //planner->Read();
        done = plannerProxy->GetPathDone();
